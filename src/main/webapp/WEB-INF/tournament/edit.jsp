<%-- 
    Document   : edit
    Created on : Feb 23, 2016, 3:10:37 PM
    Author     : eduarc
--%>
...
<h2 class="main_content_title"> Edit Tournament </h2>
<form id="newTournamentForm" role="form" class="form-horizontal" action="<%=TournamentController.URL%>" method="POST">
    <input type="hidden" name="do" value="<%=TournamentController.EDIT%>">
    <input type="hidden" name="${Param.TOURNAMENT}" value="${tournament.id}">
        ...
        <div class="col-md-4">
            <select name="${Param.SPORT}" class="form-control">
                ...
                <jsp:useBean id="sport" class="co.com.bookmaker.data_access.entity.event.Sport"></jsp:useBean>
                    <option value="${sport.id}" ${tournament.sport.id == sport.id ? "selected" : ""}>${sport.name}</option>
                </c:forEach>
            </select>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="col-md-4">
            <input type="text" class="form-control input-sm" name="${Param.NAME}" value="${tournament.name}" placeholder="Name">
        </div>
    ...
    <div class="col-md-6" style="text-align: center">
        <button class="btn btn-submit" id="btnSaveTournament"><span class="glyphicon glyphicon-save"></span> Save</button>
    </div>
</form>
